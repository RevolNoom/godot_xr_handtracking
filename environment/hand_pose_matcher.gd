...
func calculate_difference(pose1: Dictionary, pose2: Dictionary) -> float:
	...
	for bone in pose1.keys():
		var distance_to_wrist_diff = abs(pose1[bone][POSITION].length_squared() - pose2[bone][POSITION].length_squared())
		difference += distance_to_wrist_diff
		
		# TODO: One day I'll have to use handtracked_pose_capture_area to correctly 
		# store tracked-hand transform
...
func _process(_delta):
	...
	if recognized_pose != current_pose:
		...
		emit_signal("new_pose", old_pose, recognized_pose)
