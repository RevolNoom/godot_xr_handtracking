...
class_name XRGrabAreaController

signal picked_up(by_hand: Skeleton3D, at_grab_point: XRGrabArea)
...
var _grab_points: Array[XRGrabArea] = []
...
func _on_grabpoint_pick_up(by_hand: Node3D, _grabpoint: XRGrabArea) -> void:
	# Skip if disabled or already picked up
	if not can_pick_up():
		return

	_current_grabbers.push_back(by_hand)
	emit_signal("picked_up", by_hand, _grabpoint)


func GetGrabPoints() -> Array:
	var result = []
	for child in get_children():
		if child is XRGrabArea:
...
func _on_child_entered_tree(node):
	if node is XRGrabArea:
		_grab_points.append(node)
	update_configuration_warnings()


func _on_child_exiting_tree(node):
	if node is XRGrabArea:
		_grab_points.erase(node)
	update_configuration_warnings()
...
func _get_configuration_warnings():
	...
	for child in get_children():
		if child is XRGrabArea:
			return warnings
	warnings.append("Please add some children XRGrabArea.")
	return warnings
