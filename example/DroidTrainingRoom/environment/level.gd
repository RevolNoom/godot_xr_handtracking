...
@onready var infinite_wave = $SpawnPoint/SP3.duplicate()
...
func play_infinite_wave():
	var wave = infinite_wave.duplicate()
	var droids = []
	for path in [
		"Path3D/PathFollow3D/RT/SuperBattleDroid",
		"Path3D2/PathFollow3D/RT/SuperBattleDroid",
		"Path3D3/PathFollow3D/RT/SuperBattleDroid",
		"Path3D4/PathFollow3D/RT/SuperBattleDroid"]:
		droids.push_back(wave.get_node(path))
	
	$SpawnPoint.add_child(wave)
	
	this_wave_droids = droids
